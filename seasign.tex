...
\]
where the map $\Z^n \rightarrow \Cl( \OO )$ is the group homomorphism $(x_1, \dots, x_n) \mapsto \prod_i \l_i^{x_i}$.
...
A natural computational problem, that arises in the signature scheme, is to efficiently compute a short vector $(z_1, \dots, z_n)$ corresponding to a given ideal class; we discuss this problem in the next subsection.
...
We use the average-case approximations (assuming that the entries $z_i$ are uniformly distributed in $[ -\Vert \z \Vert_\infty , \Vert \z \Vert_\infty ]$) $ \Vert \z \Vert_\infty \approx \sqrt{3/n} \Vert \z \Vert_2$ and $\Vert \z \Vert_1 \approx \tfrac{n}{2} \Vert \z \Vert_\infty  \approx \sqrt{3n/4} \Vert \z \Vert_2$.
...
Once the instance $\w$ is provided one can perform: the Babai nearest plane method (or an iterative version of it, as done by Lindner and Peikert); enumeration; reducing to SVP (the Kannan embedding technique) and running a basis reduction algorithm.
...
Equation (4.3) of Babai~\cite{Bab86} shows that the near plane algorithm on input $\w$ outputs a vector $\v \in L$ with
\begin{equation} \label{eq1}
    \Vert \w -\v \Vert_2^2 \le (\Vert \bb_1^*\Vert_2^2 + \Vert \bb_2^*\Vert_2^2 + \cdots + \Vert \bb_n^*\Vert_2^2 )/4 .
\end{equation}
Bounds on $\Vert \bb_i^* \Vert$ are regularly discussed in the literature. For example, much work on the BKZ algorithm is devoted to understanding the sizes of these vectors; see Gama-Nguyen and Chen-Nguyen.
...
For our calculations we simply consider a BKZ-reduced lattice basis and, following Chen-Nguyen, assume that
\[
...
Our recommendation is for all users to share the same isogeny graph in which case the relation lattice is precomputed and optimised. 
