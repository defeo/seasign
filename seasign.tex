...
Despite the optimizations described in~\cite{DFKS18}, this technique is currently incapable of producing efficient parameters in a reasonable amount of time.
...
For simplicity, we describe this scheme in the setting of a general class group action on a set of $j$-invariants of elliptic curves.
In Section~\ref{sec:CSIDH-implementation} we explain one small subtlety that arises when implementing the scheme in the setting on CSIDH.
...
Castryck et al~\cite{CLMPR18} observe that the Montgomery model for these curves provides an elegant solution to the dilemma.
