...
Define $\D_B$ to be the distribution on $\Cl( \OO )$ corresponding to the ideal class of $\prod_{i=1}^n \l_i^{e_i}$ over uniformly sampling $\e \in [-B,B]^n$.
Understanding the distribution $\D_B$is non-trivial.
For small $B$ and $n$ we expect $\D_B$ to be the uniform distribution on a subset of $\Cl(\OO)$ of size $(2B+1)^n$. For fixed $n$ and large enough $B$ it should be the case that $\D_B$ is very close to the uniform distribution.
But a full study of the distribution is beyond the scope of this paper.
We define $M$ to be an upper bound on the probability, over $\a, \b$ sampled from $\D_B$, that $\a \cong \b$.
We will always assume that the parameters are chosen such that $M \le 1/2^\lambda$, where $\lambda$ is the security parameter.
...
When $b_k = 1$ the simulator chooses a random ideal $\c_k = \prod_{i=1}^n \l_i^{g_i}$ for $g_i \in [-ntB, ntB]$.
...
The probability of failure is at most $Q/M^t$, 
where $M$ is defined above to be an upper bound on the probability of a collision in the sampling of ideal classes.
Note that $Q/M^t$is negligible.
...
Due to the forking lemma, this proof is not tight.
(**) We could give an exact formula for the success probability of the reduction.
...
For each $k$ the prover responds with a representation $\c_k$ of the ideal class $\a_{b_k}^{-1} \b_k$ and the verifier can check that $j( E_k ) = j( \c_k * E_{A, b_k})$.
...
Since this assumption seems a bit un-natural, it is natural to try to prove security based on the problems in Definition~\ref{defn:ass1} or~\ref{defn:ass1p}.
One can try to prove security based on the problem in Definition~\ref{defn:ass1}, but it requires an assumption about uniform sampling of ideal classes.
So let $B$ be smallish and $\a = \prod_{i=1}^n \l_i^{e_i}$ where the exponent vector $\e = (e_1, \dots, e_n)$ is chosen in $[-B,B]^n \subseteq \Z^n$.
Now let $B'$ be much larger than $B$ 

Proof: We can reduce to the standard problem of Definition~\ref{defn:ass1} with an extra $1/2$ factor in the success probability (which is not a big issue since the security proof isn't tight anyway).
Let $(E, E_A = \a * E )$ be an instance of Definition~\ref{defn:ass1}. Choose random ideal classes $\b_1, \dots, \b_{2^s}$
(chosen as $\b_m = \prod_{i=1}^n \l_i^{u_{i,m}}$ for $1 \le m \le 2^s$ and $u_{i,m} \in [-B,B]$)
 and compute $E_m = \b_m * E$ for $1 \le m \le 2^{s-1}$ and $E_m = \b_m * E_A$ for $2^{s-1} < m \le 2^s$. Choose a random permutation $\pi$ on $\{ 1, 2, \dots, 2^s \}$ and set the public key to be the sequence $E_{A,m} = E_{\pi(m)}$.
...
The subtle issue is in the signing protocol; when the $k$-th block of challenge bits corresponds to $m$ we need to output a representation of an ideal that gives an isogeny from the public key curve $E_m$ to the ephemeral signing curve $E_t$.
(** BAD NOTATION CLASH **)
This will be done using rejection sampling as before.
But keep in mind that , but the ideal $\b_m$ was used to construct an isogey from $E_A$ to $E_m$, not an isogeny from $E$ to $E_m$; so the simulation is only correct if the distributions of ideal classes arising are close to uniform.
...
Below table gives some estimates of cost.
