...
Rather than storing all the private keys $\a_i$ one could have generated them using a pseudorandom generator as $\PRF( \seed, i )$ where $\seed$ is a seed and $i$ is used to generate the $i$-th private key (which is an integer exponent vector).
...
\item $\PRFm: \{0,1\}^\lambda \times \{0,1\}^{s+1} \to \{0,1\}^{\lceil\log p\rceil}$ pseudo-random functions, and
...
Like in Section~\ref{sec:private-key-compress}, we define the secret ideals $\a_u = \PRFs(\SK.\seed, u)$ and the public curves $E_{A,u}=\a_u*E$, for $1\le u\le 2^s$.
...
Mowever, in our case recomputing the leaves of the hash tree ($2^s$ class group actions) is much more expensive than recomputing the internal nodes ($2^s$ hash function evaluations), thus we set the secret key to the tuple $(\SK.\seed,h_{s,1},\dots,h_{s,2^s})$.
...
The verifier computes $E_i$ as $\c_i * E_{A,b_i}$, obtains the challenges $b_1,\dots,b_t$, and uses them to verify the authentication path.
...
We can save some space by only sending the hash values once, in some standardized order, the worst case happening when no path merges before level $\log(t)$, thus no more than $t(s-\log(t))$ hash values need to be sent as part of the signature.
...
\begin{theorem}
  In the random oracle model, the signature scheme of this section is
  unforgeable under a chosen message attack under the following assumptions:
  \begin{itemize}
  ...
  \item The pseudo-randomness of $\PRFs$, $\PRFk$ and $\PRFm$.
  \end{itemize}
\end{theorem}
...
\begin{landscape}
  \begin{table}
    ...
    \pgfmathtruncatemacro{\ComSig}{round(\ParSig + (\t * \logp  + \lam * (\lam - \t * log2(\t))) / 8)}
    % PK size
