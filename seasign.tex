...
Here's the basic idea to get a lossy scheme:
...
A \emph{canonical identification scheme} consists of algorithms $(\IGen, \PP_1, \PP_2, \VV)$ and a set $\ChSet$. The randomised algorithm $\IGen$ outputs a key pair $(pk,sk)$.
...
The scheme is \emph{commitment recoverable} if, given $c$ and $Z = P_2( sk, W, c, \St )$, there is a unique $W$ such that $\VV( pk, W, c, Z ) = 1$ and this $W$ can be efficiently computed from $(pk, c, Z)$
...
***We say $W$ has $\alpha$ \emph{bits of min entropy} if the probability any -- not needed

*** Computational unique responses (Defn 2.7 of KLS ) -- needed?

Lossy key -- definition 2.8 of KLS
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		...
		\State $(W,\St) = P_1( sk, PRF_K( 0, \msg, k ))$
		\State $c = H( W, \msg )$ 
		\State $Z = P_2( sk, W, c, \St, PRF_K( 1, \msg, k ))$
		\EndWhile
		\If{$Z = \perp$}
		\State \Return $\perp$
		\Else
...
\begin{lemma}
Let parameters satisfy the bound of equation~(\ref{eq:lossy-p-bound}). Assume heuristic 1. Let $(E,E_A)$ be a key output by the lossy key generator. Then with overwhelming probability there is no ideal $\a = \prod_{i=1}^n \l_i^{f_{i}}$ such that $\f \in [-(nt+1)B,(nt+1)B]^{2n}$ and $j(E_A) = j( \a * E )$.
\end{lemma}

\begin{proof}
Given $E$ there are at most $(2 (nt+1)B + 1)^{2n}$ choices for $\a$ and hence at most that many $j( \a * E )$. Since $E_A$ is uniformly and independently sampled from a set of size at least $\sqrt{p}$, the probability that $j(E_A)$ lies in the set of all possible $j( \a * E )$ is at most $1/2^\lambda$, which is negligible.
\end{proof}
...
This result shows that the lossy keys give secutity..
...
\begin{proof}
Fix $W$.
Suppose there are responses $Z$ and $Z'$ valid for challenges $c, c'$ respectively.
Since $c \ne c'$ there is some index $k$ such that $c_k \ne c_k'$. 
Wolog $c_k = 0$ and $c_k' = 1$. 
So $Z$ contains $\z_k$ giving an ideal $\b = \prod_i \l_i^{z_{k,i}}$ corresponding to an isogeny $E$ to $E_k$, and $Z'$ contains $z_k'$ giving an ideal $\b'$ and an isogeny from $E_A$ to $E_k$. 
Hence there is an isogeny from $E$ to $E_A$ given by $(\b')^{-1} \b$.
But by Heuristic 3 it is unlikely there exists any such ideal. 
\end{proof}

- no abort HVZK


State secutrity theoerem

Proof: Theorem 3.1 of KLS.
