...
Hence, there are subexponential-time quantum algorithms for the problem of Definition~\ref{defn:ass1} based on the quantum algorithms of Kuperberg~\cite{Kup} and Regev~\cite{regev04}. 
...
But at the very least, Kuperberg's algorithm requires at least $\tilde{O}( 2^{\sqrt{\log(p)/2}} )$ quantum gates, thus taking $p>2^{2\lambda^2}$ should be sufficient to make Definition~\ref{defn:ass1} hard for a quantum computer.


If the ideals $\a$ in Definition~\ref{defn:ass1} are sampled uniformly at random then the problem admits a random self-reduction: given an instance $(E, E_A)$ one can choose random ideal classes $\b_1, \b_2$ and construct the instance $(E_1, E_2) = (\b_1 * E, \b_2 * E_A )$, which is now uniformly distributed across the set of pairs of isomorphism classes of curves in the isogeny class.
...
This leads to a special case of Definition~\ref{defn:ass1} where the ideals may not be uniformly distibruted in the ideal class group. For further discussion  see Definition~\ref{defn:sampling-distributions} and the discussion that follows it.
...
In the random oracle model, the basic signature scheme of Figure~\ref{fig:sig-scheme} is unforgeable under a chosen message attack under the assumption of Definition~\ref{defn:ass1p}.
\end{theorem}
...
However, if the ideals $\a_m$ are not sampled uniformly at random from $\Cl(\OO)$ then we do not know how to obtain a random-self-reduction for this problem, which prevents us from having a tight reduction to the problem of Definition~\ref{defn:ass1p}.
...
Then given an algorithm that runs in time $T$ and solves the problem of Definition~\ref{defn:one-out-of-2s-problem} with probability $\epsilon$, thee is an algorithm to solve the problem of Definition~\ref{defn:ass1} with time $T + O( 2^s \log(p)^3 )$ and success probability $\epsilon/2$.
\end{lemma}

\begin{proof}
Let $A$ be an algorithm for Definition~\ref{defn:one-out-of-2s-problem}, and let $(E, E_A = \a * E )$ be an instance of Definition~\ref{defn:ass1}.
...
This computation takes $O( 2^s \log(p)^3 )$ bit operations.
...
This is where the subtlety about distributions appears: it is crucial that the curves derived from the pair $(E, E_A)$ are indistinguishable from the curves in Definition~\ref{defn:one-out-of-2s-problem}.
...
With probability $1/2$ we have that one of the curves, say $E_{m}$, is known to the simulator as  $\b * E$ and the other (i.e., $E_{m'}$) is known as $\b' * E_A$. If this event occurs then we have $\c \b * E = \b' * E_A$ (or vice versa) in which case $\c \b (\b')^{-1}$ is a solution to the original instance. \qed
\end{proof}
...
We have a tight proof in Appendix~\ref{sec:lossy-keys} based on a less standard assumption. It is an open problem to have a tight proof and also the security based on the assumption of Definition~\ref{defn:ass1}.
...
For our choices $n=50, t = 8, \sigma = 5$ this gives $\sigma' > 3000$ (experiments ongoing).
...
The best option seems to be to sample $\e$ uniformly with coefficients in $[-B, B]$ but to sample $\f$ from discrete Gaussians. Taking $n=74, B = 5$ we have $\Vert \e \vert \le T = 28$ with high probability. Taking $\sigma' = 10 \sqrt{t} T \approx 790$ potentially gives signatures of around 900 bytes.
...
\begin{definition} \label{defn:decisional-short-isogeny}
...
The signature scheme (applying Figure~\ref{fig:sign} to Figure~\ref{fig:id-scheme}) has UF-CMA security in the quantum random oracle, with a tight security reduction.
\end{theorem}
...
One can then combine this proof with the optimisations of Sections~\ref{sec:smaller-sigs} and~\ref{sec:smaller-keys}, to get a compact signature scheme with tight post-quantum security based on a merger of the assumptions in Definitions~\ref{defn:one-out-of-2s-problem} and~\ref{defn:decisional-short-isogeny}.
