...
For security against Tani's quantum algorithm we may require $(2B+1)^n > 2^{3 \lambda}$, and so $n \log(B) \sim 3 \lambda$, although the work of Adj  et al.~\cite{adj+menezes+al-isogenies} suggests this may be too cautious. In any case, we have $n \log(B) = \Omega( \lambda )$.
...
Then, $n\log(n)\sim\log(p)\sim 2\lambda^2$ and so we have  $\log(n) \sim \lambda^2 / \log(\lambda)$.
...
Note that we expect to be able to get smaller signatures at similar cost, for example see the various options in Table~\ref{tab:alt-params}.
