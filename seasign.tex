...
\begin{table}
  ...
  \pgfmathtruncatemacro{\ParTime}{\t * \CSIDHsecs * 10}
  \pgfmathtruncatemacro{\ComTime}{\ParTime}
  \centering
  \begin{tabular}{l | c | c | c |}
    & Stolbunov scheme
    ...
    Est. sig/verify time & \StoTime{} s & 0.\ParTime{} s & 0.\ComTime{} s
  \end{tabular}
  \caption{Signature size and sign/verify performance of Stolbunov's
    ...
    asymptotic analysis,
    $(\lambda,n,B,\log_2(p)) = (128, 74, 5, 16, 500)$ with
...
We end by mentioning a suggestion of Stolbunov~\cite{Sto12} that will give signatures of optimal size when the relation lattice is known.
%
% he gives a one sentence sketch of a solution that is applicable when the discrete logs of the primes $\l_i$ in the class group are known.
%
Suppose the ideal class group is cyclic of order $N$ and let $\g$ be a generator.
...
The verifier just sees a uniformly distributed integer modulo $N$, and uses this to recompute $\E_k$ from either $E$ or $E_A$.
